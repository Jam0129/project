$(document).ready(function () {
  ...
  $dots1.eq(0).addClass('active');
  $dots2.eq(0).addClass('active');
  $dots3.eq(0).addClass('active');
  $dots4.eq(0).addClass('active');
  $dots5.eq(0).addClass('active');
  $dots6.eq(0).addClass('active');
  ...
  $nextIcons.click(function () {
    ...
    let dots = container.find('.dots1 span, .dots2 span, .dots3 span, .dots4 span, .dots5 span, .dots6 span');
  ...
  $('.image-container img, .image-container video').dblclick(function(){
    ...
    let heartIcon = imageContainer.nextAll('.icon').find('.fa-heart'); // 다음에 오는 icon 클래스 내 하트 아이콘 찾기
    let likeText = imageContainer.nextAll('.icon').next().find('h4'); // 좋아요 텍스트 찾기
  ...
  // 좋아요 버튼 이벤트 (게시글)
  $('.commentheart, .commentheart2, .commentheart3, .commentheart4, .commentheart5, .commentheart6').click(function () {
    let heartIcon = $(this);
    let iconContainer = heartIcon.closest('.icon'); // 해당 하트 아이콘이 속한 아이콘 컨테이너 찾기
    let likeText = iconContainer.next().find('h4'); // 해당 아이콘 컨테이너 다음의 좋아요 텍스트 찾기
    let count = parseInt(likeText.text().match(/\d+/)?.[0] || '0', 10);
  ...
  $('.replyheart').click(function () {
    let heartIcon = $(this);
    let likey = $(this).closest('.otheruser').find('.likey');
